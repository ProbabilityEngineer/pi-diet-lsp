...
async function run(cmd: string, args: string[], cwd?: string): Promise<{ code: number | null; stdout: string; stderr: string }> {
  return await new Promise((resolve) => {
    ...
    let stdout = "";
    let stderr = "";
    child.stdout.on("data", (d) => { stdout += String(d); if (stdout.length > MAX_OUTPUT) child.kill(); });
    child.stderr.on("data", (d) => { stderr += String(d); if (stderr.length > MAX_OUTPUT) child.kill(); });
...
async function sg(args: string[], cwd?: string) {
  ...
  if (res.code === 127) res = await run("npx", ["--yes", "@ast-grep/cli", ...args], cwd);
  return res;
}
...
function formatSgJson(stdout: string, stderr: string) {
  ...
  try {
    ...
    return arr.slice(0, 100).map((m: any) => {
...
class LspClient {
  ...
  start() {
    if (this.proc) return;
    this.proc = spawn(this.command, this.args, { cwd: this.cwd, stdio: "pipe" });
    this.proc.stdout.on("data", (chunk) => this.onData(chunk));
    this.proc.on("exit", () => { this.proc = undefined; this.initialized = false; });
  }
  ...
  private onData(chunk: Buffer) {
    ...
    while (true) {
      ...
      if (!match) { this.buffer = this.buffer.subarray(sep + 4); continue; }
  ...
  notify(method: string, params: Json) { this.send({ jsonrpc: "2.0", method, params }); }

  async ensure() {
    if (this.initialized) return;
    await this.request("initialize", { processId: process.pid, rootUri: pathToFileURL(this.cwd).href, capabilities: {} });
  ...
  getDiagnostics(uri?: string) {
    ...
    return [...this.diagnostics.entries()].flatMap(([u, ds]) => ds.map((d) => ({ ...d, uri: u })));
  }
}
...
function languageIdFor(file: string) {
  ...
  if ([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"].includes(ext)) return ext.includes("ts") ? "typescript" : "javascript";
...
function serverFor(file: string): { command: string; args: string[] } | undefined {
  const ext = path.extname(file);
  if ([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"].includes(ext)) return { command: "typescript-language-server", args: ["--stdio"] };
  if (ext === ".py") return { command: "pyright-langserver", args: ["--stdio"] };
  if (ext === ".go") return { command: "gopls", args: [] };
  if (ext === ".rs") return { command: "rust-analyzer", args: [] };
  if ([".json", ".jsonc"].includes(ext)) return { command: "vscode-json-language-server", args: ["--stdio"] };
  if ([".yml", ".yaml"].includes(ext)) return { command: "yaml-language-server", args: ["--stdio"] };
  return undefined;
}

async function getClient(cwd: string, filePath: string) {
  const spec = serverFor(filePath);
  if (!spec) throw new Error(`No lightweight LSP server mapping for ${path.extname(filePath) || filePath}`);
  const key = `${cwd}:${spec.command}`;
  let c = clients.get(key);
  if (!c) { c = new LspClient(spec.command, spec.args, cwd); clients.set(key, c); }
  return c;
}

function lspPos(line: number, character: number) { return { line: Math.max(0, line - 1), character: Math.max(0, character - 1) }; }
function pretty(value: Json) { return typeof value === "string" ? value : JSON.stringify(value, null, 2); }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({ pattern: Type.String(), rewrite: Type.String(), lang: Type.String({ enum: [...LANGS] as string[] }), paths: Type.Array(Type.String()), apply: Type.Optional(Type.Boolean()) }),
    async execute(_id: string, params: any, _signal: AbortSignal, _update: unknown, ctx: ToolCtx) {
      const args = ["run", "-p", params.pattern, "-r", params.rewrite, "--lang", params.lang];
      if (params.apply) args.push("--update-all"); else args.push("--json=compact");
      args.push(...params.paths);
      const res = await sg(args, ctx.cwd);
      return text(params.apply ? (res.stderr || res.stdout || "Applied") : formatSgJson(res.stdout, res.stderr), { code: res.code, applied: !!params.apply });
    },
  } as any);
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({ filePath: Type.String(), line: Type.Number(), character: Type.Number() }),
    async execute(_id: string, p: any, _s: AbortSignal, _u: unknown, ctx: ToolCtx) {
      const file = resolvePath(ctx.cwd, p.filePath); const cwd = ctx.cwd ?? process.cwd(); const c = await getClient(cwd, file); const uri = await c.open(file);
      const result = await c.request("textDocument/definition", { textDocument: { uri }, position: lspPos(p.line, p.character) });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({ filePath: Type.String(), line: Type.Number(), character: Type.Number(), includeDeclaration: Type.Optional(Type.Boolean()) }),
    async execute(_id: string, p: any, _s: AbortSignal, _u: unknown, ctx: ToolCtx) {
      const file = resolvePath(ctx.cwd, p.filePath); const cwd = ctx.cwd ?? process.cwd(); const c = await getClient(cwd, file); const uri = await c.open(file);
      const result = await c.request("textDocument/references", { textDocument: { uri }, position: lspPos(p.line, p.character), context: { includeDeclaration: p.includeDeclaration ?? true } });
      return text(pretty(result));
    },
  } as any);
  ...
  pi.registerTool({
    ...
    async execute(_id: string, p: any, _s: AbortSignal, _u: unknown, ctx: ToolCtx) {
      const file = resolvePath(ctx.cwd, p.filePath); const cwd = ctx.cwd ?? process.cwd(); const c = await getClient(cwd, file); const uri = await c.open(file);
      const result = await c.request("textDocument/hover", { textDocument: { uri }, position: lspPos(p.line, p.character) });
      return text(pretty(result));
    },
  } as any);
  ...
  pi.registerTool({
    name: "lsp_symbols",
    label: "LSP Symbols",
    description: "List document symbols for a file, optionally filtered by query.",
    parameters: Type.Object({ filePath: Type.String(), query: Type.Optional(Type.String()) }),
    async execute(_id: string, p: any, _s: AbortSignal, _u: unknown, ctx: ToolCtx) {
      const file = resolvePath(ctx.cwd, p.filePath); const cwd = ctx.cwd ?? process.cwd(); const c = await getClient(cwd, file); const uri = await c.open(file);
      const result = await c.request("textDocument/documentSymbol", { textDocument: { uri } });
      const q = String(p.query ?? "").toLowerCase();
      const filtered = q && Array.isArray(result) ? result.filter((s: any) => JSON.stringify(s).toLowerCase().includes(q)) : result;
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({ filePath: Type.String(), waitMs: Type.Optional(Type.Number()) }),
    async execute(_id: string, p: any, _s: AbortSignal, _u: unknown, ctx: ToolCtx) {
      const file = resolvePath(ctx.cwd, p.filePath); const cwd = ctx.cwd ?? process.cwd(); const c = await getClient(cwd, file); const uri = await c.open(file);
      await new Promise((r) => setTimeout(r, Math.max(0, Math.min(5000, p.waitMs ?? 800))));
